...
const QUEUE_NAME = 'tagfy-queue-tasks';
...
async function getProductsToMine() {
    ...
    try {
        ...
        const query = `
            SELECT id, url, store, user_id, price, last_mined_at, next_mine_at
            FROM monitors 
            WHERE next_mine_at <= (CURRENT_TIMESTAMP AT TIME ZONE 'America/Sao_Paulo')
